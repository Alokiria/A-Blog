...
<template>
    <Transition enter-active-class="animate-fade-in">
        ...
        <!-- v-if="showMenu" 受首页动画控制 -->
        <div v-if="showMenu" class="yun-nav-menu z-$yun-z-nav-menu fixed bg-transparent" :class="{
            ...
            <div class="inline-flex justify-start items-center flex-1">
                ...
                <!-- 其他组件，默认隐藏，只有在大屏情况下才显示 -->
                <!-- <div class="hidden lg:contents">
                    <YunNavMenuItem v-for="item, i in themeConfig.nav" :key="i" :icon="item.icon" :to="item.link"
                        :title="$t(item.text)" />
                </div> -->
            </div>
          ...
          <template v-for="item in themeConfig.nav" :key="item.text">
            ...
            <template v-else-if="'items' in item" />
                ...
                <!-- 多语言切换，仅大屏显示 -->
                <YunToggleLocale v-if="app.showToggleLocale" class="rounded-none! hidden sm:inline-flex" />
